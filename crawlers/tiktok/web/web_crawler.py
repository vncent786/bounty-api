...
class TikTokWebCrawler:
    ...
    # 搜索视频 (Search Videos by Keyword)
    async def fetch_search_videos(self, keyword: str, offset: int = 0, count: int = 12,
                                   search_id: str = "", sort_type: int = 0,
                                   publish_time: int = 0):
        ...
        async with base_crawler as crawler:
            ...
            endpoint = BogusManager.model_2_endpoint(
                TikTokAPIEndpoints.SEARCH_ITEM, params,
    ...
    async def fetch_search_users(self, keyword: str, offset: int = 0, count: int = 10):
        kwargs = await self.get_tiktok_headers()
        base_crawler = BaseCrawler(proxies=kwargs["proxies"], crawler_headers=kwargs["headers"])
        async with base_crawler as crawler:
            params = {
            ...
            endpoint = BogusManager.model_2_endpoint(
                TikTokAPIEndpoints.SEARCH_USER, params,
                kwargs["headers"]["User-Agent"]
            )
            response = await crawler.fetch_get_json(endpoint)
        return response
