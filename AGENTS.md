...
## Source-health debugging

When X, TikTok, Instagram, Reddit, YouTube or Google Trends is described as rate-limited, unhealthy, empty, partial, timed out or stale, load the Hermes skill `bounty-source-health-debugging` before retrying or reporting. Follow `references/social-source-health-debugging.md` and classify the exact failed layer. Do not collapse HTTP 400, profile contention, local account budget, credential/session failure, timeout, parser drift, healthy no-match or one unreadable thread into “rate limited.” A fix is not complete until the source-only two-cycle qualification passes before any monitor/dashboard release.

## What this is

Bounty's public Investor Radar is withdrawn while its signal methodology is rebuilt. `/dashboard` serves stable Classic Bounty; `/dashboard/investing-preview` is the internal private Radar. The private Radar has an owned-worker scan that persists progress. Its Opportunity lane preserves specific cited behavior from the last 90 days with missing evidence and next actions; only its trade-ready lane requires deterministic specificity, behavior, comparable-history anomaly, breadth, citation, parity, depth, and investigability gates. Its authenticated Research view now turns an exact persisted Radar candidate into a saved, hash-verified company dossier: human-confirmed company/ticker, free GLEIF/OpenFIGI resolution, automatic SEC filing and CompanyFacts research for SEC registrants, candidate-specific filing passages, optional transcript sources, sampled public-news checks, and explicit low/base/high analyst assumptions. Common stock is valid and options are optional. Non-US filings without SEC coverage use user-supplied primary URLs and remain explicit source candidates rather than auto-verified numeric facts. `No qualified leads` and `materiality not estimable` are valid results. Public Social Pulse scheduling stays off. Source notes: `references/camillo-information-arbitrage-methodology-2026-08.md`; transparent current filter policy: `references/private-radar-filter-policy-2026-08.md`; dossier contract: `.hermes/plans/2026-08-29_170000-generic-candidate-dossier-release.md`.
...
- **Owned TikTok/Instagram worker:** live search, comments, and replies work on the Windows residential host. Every private-Radar sweep now preflights both depth routes, deduplicates thread roots across panels, prefers unseen roots, and records bounded-sample coverage separately from full-platform totals. TikTok reads up to 60 records with a 40-record sufficiency target; Instagram reads up to 60 with a 30-record target because its authenticated web surface commonly exposes 15 ranked roots plus reply chains. Truncated source conversations remain explicit. The signed ingestion handoff, automatic/manual re-auth alerting, second-worker failover, and seven-day canary are not built yet; do not imply an external SLA.
